...
import { transCode, localeWordPattern } from "./transform.js";
import { setConfig, setCurrentCompileResourceMap } from "../common/collect.js";
...
export default function i18nTransform({ id, code }, options) {
  ...
  function isInConsole(path) {
    ...
    if (
      ...
      if (
        parentCalleeObject.type === "Identifier" &&
        parentCalleeObject.name === "console"
      ) {
        return true;
      }
    }
  ...
  const visitor = {
    ...
    VariableDeclarator(path) {
      ...
      if (
        initNodeCallee.type === "Identifier" &&
        initNodeCallee.name === "require"
      ) {
      ...
      if (dependency.objectPattern) {
        ...
        if (
          ...
          path.node.id.name === dependency.name
        ) {
          nameMatched = true;
        }
      }
    ...
    TemplateLiteral(path) {
      ...
      const hasWord = path.node.quasis.some((item) =>
        localePattern.test(item.value.raw),
      );
      if (!hasWord) {
        return;
      }
      ...
      path.node.quasis.forEach((node) => {
        ...
        if (string) {
          ...
          const unshiftIndex = sections.findIndex(
            (item) => node.start < item.start,
          );
          unshiftIndex === -1
            ? sections.push(element)
            : sections.splice(unshiftIndex, 0, element);
        }
      });
