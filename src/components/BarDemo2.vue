...
const currentDay = currentDate.getDate(); // 当前日期几号
...
const provinces = computed(() => {
  return serverData.value.data.map(
    (item) => item.provinceName + "这种自知则知之",
  );
});
...
const provinceProgress = computed(() => {
  return serverData.value.data.map((item) => {
    return (
      (item.signingCompletedQuantity / item.targetQuantity) *
      100
    ).toFixed(2);
  });
});
const maxLabelWidth = 56; // 24px宽度换行
onMounted(() => {
  ...
  option = {
    ...
    xAxis: {
      ...
      axisLabel: {
        ...
        formatter: (value) => wrapLabel(value, 3), // 处理标签换行

        rich: {
          label: {
            width: maxLabelWidth,
            overflow: "break", // This might not be necessary but ensure text wraps
            align: "center",
          },
        },
      },
    ...
    series: [
          ...
          data: [
            {
              ...
              label: {
                show: false,
                formatter: `时间进度: ${timeProgress.toFixed(2)}%`,
              },
              lineStyle: {
                color: " #CAAC7B", // 线的颜色
                type: "solid", // 线的样式，可以是 'solid', 'dashed', 'dotted'
              },
            },
          ],
        },
      ...
      {
        // 伪造一个 series 用于在 legend 中显示时间进度
        name: "时间进度",
        type: "line",
        data: [], // 空数据
        markLine: {
  ...
  myChart.on("legendSelectChanged", function (params) {
    const selected = params.selected["时间进度"];
    const seriesIndex = option.series.findIndex(
      (series) => series.name === "招商进度",
    );

    if (seriesIndex !== -1) {
      // 更新标记线的显示状态
      option.series[seriesIndex].markLine.data[0].lineStyle.opacity = selected
        ? 1
        : 0;
